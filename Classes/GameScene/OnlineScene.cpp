...
//谁进来就代表哪个玩家
static Client* my_client = nullptr;

extern int online_id;

Scene* OnlineScene::createScene(Client* client)
{
	my_client = client;
...
bool  OnlineScene::init()
{
	...
	//第一个人随便选，但是第二个人一定是妲己，因为数据传输可能有点问题
	std::vector<std::string> id_messages = my_client->get_messages();
	for (int i = 0; i != id_messages.size(); ++i)
	{
		if (id_messages[i].size() < 2)
		{
			continue;
		}
		if (id_messages[i][1] == AI_HERO_TYPE)
		{
			std::string message = id_messages[i];
			std::string cut_head_message = message.substr(2);
			std::stringstream ss(cut_head_message);
			ss >> ai_id;
		}
	}
	ai_id += 3;

	if (my_client->_head == "A")
	{
		//我方英雄
		my_hero = HeroSprite::create(online_id);
		this->addChild(my_hero);
		...
		ai_hero->setPosition(Vec2(5460, 700));
		ai_hero->setAnchorPoint(Vec2(0.5, 0.5));
	...
	else
	{
		...
		my_hero = HeroSprite::create(online_id);
		this->addChild(my_hero);
		my_hero->setPosition(Vec2(960, 700));
		my_hero->setAnchorPoint(Vec2(0.5, 0.5));

		map_layer->setPosition(Vec2(-4500, 0));

		ai_hero = HeroSprite::create(ai_id);
		map_layer->addChild(ai_hero);
		ai_hero->setPosition(Vec2(960, 540));
		ai_hero->setAnchorPoint(Vec2(0.5, 0.5));
			...
			for (auto& i : red_soldier_vec)
			{
				...
				*/
				if (i->boundingBox().containsPoint(touch_position) && i->getPosition().distance(my_hero_position - map_layer_position) <= my_hero->hero.attack_distance && i->soldier.survival == true)
				{
					...
					auto hero_attack_effect = AttackEffect::create(online_id);
	...
	schedule(schedule_selector(OnlineScene::time), 1);
	schedule(schedule_selector(OnlineScene::gameOver));
	schedule(schedule_selector(OnlineScene::tower_attack), 1.0f);
	schedule(schedule_selector(OnlineScene::resurrection), 1.0f);
	schedule(schedule_selector(OnlineScene::display_resurrection));
	schedule(schedule_selector(OnlineScene::ai_soldiers_attack));//攻击速度写死了
	schedule(schedule_selector(OnlineScene::level));
	schedule(schedule_selector(OnlineScene::experience));
	schedule(schedule_selector(OnlineScene::money));
	schedule(schedule_selector(OnlineScene::myhero_be_killed));
	schedule(schedule_selector(OnlineScene::send_message));
	schedule(schedule_selector(OnlineScene::update));

	return true;
}

//发送相应的信息
void OnlineScene::send_message(float)
{
	//发送我的位置
	std::string my_location = LOCATION + std::to_string(my_hero->getPosition().x - map_layer->getPosition().x) + " " + std::to_string(my_hero->getPosition().y);
	my_client->send_message(my_location);

	//发送敌方英雄的血量
	std::string ai_HP = AI_HERO_HP + std::to_string(ai_hero->hero.HP);
	my_client->send_message(ai_HP);

	//发送敌方第一个防御塔血量
	std::string tower_1_HP = AI_TOWER_HP_1 + std::to_string(towers[2]->tower.HP);
	my_client->send_message(tower_1_HP);

	//发送敌方第2个防御塔血量
	std::string tower_2_HP = AI_TOWER_HP_2 + std::to_string(towers[3]->tower.HP);
	my_client->send_message(tower_2_HP);

	//发送敌方小兵血量 附带编号
	for (int i=0;i!=red_soldier_vec.size();++i)
	{
		std::string soldier_HP = SOLDIER_HP + std::to_string(i)+" "+ std::to_string(red_soldier_vec[i]->soldier.HP);
		my_client->send_message(soldier_HP);
	}
}

//接受对方的信息，并更新数据
void OnlineScene::update(float)
{
	std::vector<std::string> messages = my_client->get_messages();

	for (int i = 0; i != messages.size(); ++i)
	{
		if (messages[i].size() < 2)
		{
			continue;
		}
		...
		case AI_HERO_HP:
		{
			...
			if (my_hero->hero.HP > 0)
			{
				my_hero->blood->setScaleX((my_hero->hero.HP / my_hero->hero.FullHP) * 0.5);
			}
			else
			{
				my_hero->blood->setScaleX(0);
				my_hero->setVisible(false);
				my_hero->hero.survival = false;
			}
		}
		break;
		case AI_TOWER_HP_1:
		{
			double tower_HP;
			ss >> tower_HP;
			towers[1]->tower.HP = tower_HP;

			//还活着，让血条相应变化
			if (towers[1]->tower.HP > 0)
			{
				towers[1]->blood->setScaleX(towers[1]->tower.HP / 9000.0 * 0.5);
			}
			//死了，血条消失，图片消失，逻辑为死亡
			else
			{
				towers[1]->blood->setScaleX(0);
				towers[1]->setVisible(false);
				towers[1]->tower.survival = false;
			}
		}
		break;
		case AI_TOWER_HP_2:
		{
			double tower_HP;
			ss >> tower_HP;
			towers[0]->tower.HP = tower_HP;

			//还活着，让血条相应变化
			if (towers[0]->tower.HP > 0)
			{
				towers[0]->blood->setScaleX(towers[0]->tower.HP / 6000.0 * 0.5);
			}
			//死了，血条消失，图片消失，逻辑为死亡
			else
			{
				towers[0]->blood->setScaleX(0);
				towers[0]->setVisible(false);
				towers[0]->tower.survival = false;
			}
		}
		break;
		case SOLDIER_HP:
		{
			...
			double HP;
			ss >> HP;
			if (blue_soldier_vec.size() > index)
			{
				blue_soldier_vec[index]->soldier.HP = HP;

				if (blue_soldier_vec[index]->soldier.HP > 0)//还活着，让血条相应变化
				{
					blue_soldier_vec[index]->blood->setScaleX((blue_soldier_vec[index]->soldier.HP / SoldierSprite::get_soldier_hp(index, time_minute)) * 0.5);
				}
				else//死了，血条消失，图片消失，逻辑为死亡
				{
					blue_soldier_vec[index]->blood->setScaleX(0);
					blue_soldier_vec[index]->setVisible(false);
					blue_soldier_vec[index]->soldier.survival = false;
				}
			}
		}
		break;
...
//小兵ai
void OnlineScene::ai_soldiers_attack(float)
{
	//对于蓝方每个小兵
...
//为的是即刻显示复活标签，不用等那一秒
void OnlineScene::display_resurrection(float)
{
...
//复活相关
void OnlineScene::resurrection(float)
{
...
//防御塔攻击
void OnlineScene::tower_attack(float)
{
...
//判断游戏结束
void OnlineScene::gameOver(float)
{
...
//添加防御塔
void OnlineScene::addTower(bool is_red)
{
	if (is_red == false)
	{
		for (int i = 0; i < 4; i++)
	...
	else
	{
		...
		//0 1 是我方塔，因为是红色，所以位置有区别了
		towers[3]->setPosition(Vec2(1550, 650));
		towers[2]->setPosition(Vec2(2550, 650));
		towers[1]->setPosition(Vec2(3750, 650));
		towers[0]->setPosition(Vec2(4950, 650));
	}
	//防御塔范围的一个圈
...
void OnlineScene::time(float)
{
	...
	else
	{
		if (time_second == 1)//近战                           
		{
			...
			map_layer->addChild(blue_soldier);
			blue_soldier_vec.push_back(blue_soldier);
			blue_soldier->setPosition(Vec2(4900, 650));

			auto red_soldier = SoldierSprite::create(3, time_minute);
			map_layer->addChild(red_soldier);
			red_soldier_vec.push_back(red_soldier);
			red_soldier->setPosition(Vec2(1600, 650));
		}
		if (time_second == 3)//远程                               
		{
			auto blue_soldier = SoldierSprite::create(1, time_minute);
...
//英雄掉血
void OnlineScene::dropBlood(HeroSprite* hero, int attack)
{
...
//英雄自身回血                                             
void OnlineScene::reBlood(HeroSprite* hero)
{
...
//塔掉血
void OnlineScene::dropBlood(TowerSprite* tower, int attack, int id)
{
...
//创建商店按钮                   
bool  OnlineScene::create_shop()
{

	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	//添加进入商店游戏按钮  
	auto item1 = MenuItemImage::create("shop/shop.png", "shop/shopselected.png", CC_CALLBACK_1(OnlineScene::OpenShop, this));
...
//打开商店回调函数
void OnlineScene::OpenShop(Ref* pSender)
{
	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	...
	if (shop_layer == NULL)
	{
		...
		//添加关闭商店游戏按钮  
		auto item1 = MenuItemImage::create("shop/close.png", "shop/closeselected.png", CC_CALLBACK_1(OnlineScene::CloseShop, this));
		...
		//添加切换商店页数按钮
		auto item_2 = MenuItemImage::create("shop/second_page.png", "shop/second_pageselected.png", CC_CALLBACK_1(OnlineScene::second_page, this));
...
//关闭商店界面
void OnlineScene::CloseShop(Ref* pSender)
{
	if (buy_it_menu != NULL)
	{
		buy_it_menu->removeFromParent();
...
//第二页商店回调函数
void OnlineScene::second_page(Ref* pSender)
{
	//先使第一页全不可见
	if (buy_it_menu != NULL)
	{
	...
	if (shop_second_layer == NULL)
	{
		...
		//添加关闭商店游戏按钮  
		auto item1 = MenuItemImage::create("shop/close.png", "shop/closeselected.png", CC_CALLBACK_1(OnlineScene::CloseShop_1, this));
		...
		//添加切换到第一页商店页数按钮
		auto item_1 = MenuItemImage::create("shop/first_page.png", "shop/first_pageselected.png", CC_CALLBACK_1(OnlineScene::first_page, this));
...
//第一页商店回调函数
void OnlineScene::first_page(Ref* pSender)
{
	if (buy_it_menu != NULL)
	{
		buy_it_menu->removeFromParent();
...
//从第二页关闭商店界面
void OnlineScene::CloseShop_1(Ref* pSender)
{
	if (buy_it_menu != NULL)
	{
		buy_it_menu->removeFromParent();
...
//显示经验
void OnlineScene::experience(float)
{
	std::string exp_text;
	...
	if (ai_hero->hero.survival == true)
		hero_survive = 1;
	if (ai_hero->hero.survival == false && hero_survive == 1)
	{
	...
	if (towers[3]->tower.survival == true)
		tower_survive_2 = 1;
	if (towers[2]->tower.survival == false && tower_survive_1 == 1)
	{
		my_hero->hero.hero_exp += 100;
		my_hero->hero.money += 120;
		tower_survive_1 = 0;
	}
	if (towers[3]->tower.survival == false && tower_survive_2 == 1)
	{
		my_hero->hero.hero_exp += 100;
		my_hero->hero.money += 120;
		tower_survive_2 = 0;
...
//显示金币
void OnlineScene::money(float)
{
	money_label->setString(std::to_string(my_hero->hero.money));
}

//更新战绩数据
void OnlineScene::myhero_be_killed(float)
{
	//被击杀数
	if (my_hero->hero.survival == true)
		OnlineScene::myhero_survive = 1;
	if (my_hero->hero.survival == false && OnlineScene::myhero_survive == 1)
	{
		++be_killed_result;
		OnlineScene::myhero_survive = 0;
	}

	//击杀数
	if (ai_hero->hero.survival == true)
		OnlineScene::aihero_survive = 1;
	if (ai_hero->hero.survival == false && OnlineScene::aihero_survive == 1)
	{
		++killed_hero_result;
		OnlineScene::aihero_survive = 0;
	}
...
//创建战绩界面
bool OnlineScene::create_CombatResult()
{

	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	//添加查看战绩游戏按钮  
	auto item1 = MenuItemImage::create("combat_result/check_result.png", "combat_result/check_result_selected.png", CC_CALLBACK_1(OnlineScene::CheckResult, this));
...
//打开战绩界面
void OnlineScene::CheckResult(Ref* pSender)
{
	auto visibleSize = Director::getInstance()->getVisibleSize();
	Vec2 origin = Director::getInstance()->getVisibleOrigin();
	...
	//添加关闭按钮  
	auto item1 = MenuItemImage::create("combat_result/close.png", "combat_result/close_selected.png", CC_CALLBACK_1(OnlineScene::CloseResult, this));
...
//关闭战绩界面
void OnlineScene::CloseResult(Ref* pSender)
{
